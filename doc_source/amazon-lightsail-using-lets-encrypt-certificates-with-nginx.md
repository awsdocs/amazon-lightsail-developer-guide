...
+ [Step 7: Configure a cron job that renews your certificate](#configure-a-cron-job-that-renews-certs-nginx)
+ [Step 8: Move the Let’s Encrypt certificate files to the Nginx server directory](#move-the-lets-encrypt-certificate-files-to-nginx)
+ [Step 9: Configure HTTP to HTTPS redirection for your web application](#configure-http-to-https-redirection-nginx)
...
![\[SSH quick connect on the Lightsail home page.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-nginx-ssh-quick-connect.png)
...
1. After your Lightsail browser\-based SSH session is connected, enter the following command to update the packages on your instance:
   ...
   sudo apt-get update
   ```  
![\[Update the packages on your instance.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-nginx-ssh-lets-encrypt-update-packages.png)
...
![\[Confirm the the domain environment variables.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-ssh-lets-encrypt-confirm-variables.png)
...
1. Let’s Encrypt now prompts you to verify that you own the domain specified\. You do this by adding TXT records to the DNS records for your domain\. Two sets of TXT record values are provided as shown in the following example:  
![\[TXT records for Let's Encrypt certificates.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-ssh-lets-encrypt-text-records.png)
...
![\[DNS zone editor in the Lightsail console.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-dns-zone-editor.png)
...
![\[TXT records in the Lightsail DNS zone editor.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-dns-zone-editor-text-records.png)
...
1. Enter the following text into the text box\. Be sure to replace `domain` with your domain\.
   ...
   ```  
![\[MXTookbox TXT record lookup.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-mxtoobox-text-record-lookup.png)
...
![\[Confirmation that TXT records propagated.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-mxtoobox-propagated-text-record-lookup.png)
...
![\[Successful Let's Encrypt cretificate request.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-ssh-lets-encrypt-request-success.png)
...
![\[Let's Encrypt certificate renewal date.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-ssh-lets-encrypt-renewal-date.png)

1. Now that you have the Let’s Encrypt SSL certificate, continue to the [next section](#configure-a-cron-job-that-renews-certs-nginx) of this tutorial\.

## Step 7: Configure a cron job that renews your certificate<a name="configure-a-cron-job-that-renews-certs-nginx"></a>
...
1. Add the following line to the end of the crontab file\.
   ...
   ```
   0 1 * * * sudo certbot renew && sudo /opt/bitnami/ctlscript.sh restart nginx
   ```
...
1. Continue to the [next section](#move-the-lets-encrypt-certificate-files-to-nginx) of this tutorial\.

## Step 8: Move the Let’s Encrypt certificate files to the Nginx server directory<a name="move-the-lets-encrypt-certificate-files-to-nginx"></a>

Move your new Let’s Encrypt SSL certificate files to the Nginx server directory on your Nginx instance\. Also, back up your existing certificates, in case you need them later\.

**To move the Let’s Encrypt certificate files to the Nginx server directory**
...
![\[Instance services stopped.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-nginx-ssh-stop-services.png)
...
1. Enter the following command to confirm the variables return the correct values:
   ...
   You should see a result similar to the following:  
![\[Confirm the the domain environment variable.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-ssh-lets-encrypt-confirm-domain-variable.png)
...
1. Enter the following commands individually to copy your Let’s Encrypt certificate files to the Nginx server directory:
...
1. Enter the following command to start the underlying services that you had stopped earlier:
   ...
   You should see a result similar to the following:  
![\[Instance services started.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-nginx-ssh-start-services.png)
...
## Step 9: Configure HTTP to HTTPS redirection for your web application<a name="configure-http-to-https-redirection-nginx"></a>
...
**To integrate your Let’s Encrypt certificate with your Nginx instance**
...
![\[Nginx configuration file edited for HTTP to HTTPs redirection.\]](https://d9yljz1nd5001.cloudfront.net/en_us/1cade0c7e07039bf59652df47a09d228/images/amazon-lightsail-nginx-ssh-lets-encrypt-config-file.png)
